#!/bin/bash
# 将 isso 的 nginx 片段写入 nginx 配置目录，不覆盖原机 site。
# 用户只需在自己的 site 里加一行 include。

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SNIPPET_SRC="$SCRIPT_DIR/isso-snippet.conf"
NGINX_SNIPPET="/etc/nginx/snippets/isso.conf"

# 由 isso.sh 传入或使用默认值
RUN_PORT="${RUN_PORT:-3500}"
REVERSE_PROXY_URL="${REVERSE_PROXY_URL:-/isso/}"
...
echo "在要用 isso 的那个 server { } 内加一行："
echo ""
echo "    include /etc/nginx/snippets/isso.conf;"
echo ""
