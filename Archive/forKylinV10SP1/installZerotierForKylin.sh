...
if [ $ID == "debian" ] || [ $ID == "raspbian" ]; then
    ...
    # 银河麒麟
	echo '*** Detected Kylin Linux(Debian 10/11), creating /etc/apt/sources.list.d/zerotier.list'
    # echo "$VERSION_ID" v10
	# 是v10才会运行
	if [ "$VERSION_ID" == "v10" ]; then
		# kylin 12 http://download.zerotier.com/ bookworm
		# write_apt_repo $ID $MAX_SUPPORTED_DEBIAN_VERSION $ZT_BASE_URL_HTTP $MAX_SUPPORTED_DEBIAN_VERSION_NAME
		# deb http://download.zerotier.com/debian/bullseye bullseye main
		# echo $ID $VERSION_ID $ZT_BASE_URL_HTTP $VERSION_CODENAME
		# kylin v10 http://download.zerotier.com/ kylin
		write_apt_repo debian 11 $ZT_BASE_URL_HTTP bullseye
	else 
	    echo "似乎不是银河麒麟V10系统?"
		exit 1
	fi
