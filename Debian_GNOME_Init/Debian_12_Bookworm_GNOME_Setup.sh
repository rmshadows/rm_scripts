...
if [ "$SET_INSTALL_NGINX" -eq 1 ];then
    ...
    if [ $? -eq 0 ];then
        ...
        echo "$NGINX_GLOBAL_CONF" | sudo tee /etc/nginx/nginx.conf
        echo "$NGINX_BLOCK_IP" | sudo tee /etc/nginx/block_ip.conf
        prompt -i "Genarate a http website."
        echo "$NGINX_HTTP_SITE" | sudo tee /etc/nginx/sites-available/http
        # 禁用默认的配置文件，启用新的
