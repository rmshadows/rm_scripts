...
if [ -d hackchat ]; then
    echo "hackchat 目录已存在，跳过 clone；若需重装请先删除该目录。"
    cd hackchat
else
...
# 修改 client/client.js 的 wsPath 用于配置反向代理
# 确保文件存在
...
if [ -f setupNginxForHackchat.sh ]; then
    prompt -x "运行 setupNginxForHackchat.sh（写入 /etc/nginx/snippets/hackchat.conf）"
    export RUN_PORT REVERSE_PROXY_URL REVERSE_PROXY_PATH
    # WebSocket 端口（hackchat 默认 6060，若不同请在 CONF 中增加 WS_PORT 并 export）
    export WS_PORT="${WS_PORT:-6060}"
    bash setupNginxForHackchat.sh
else
    prompt -w "未找到 setupNginxForHackchat.sh，请手动运行以写入 nginx 片段。"
fi
...
prompt -i "========================================================"
prompt -i "若使用片段方式，只需在自己的 site 里加一行： include /etc/nginx/snippets/hackchat.conf;"
prompt -w "若使用 nginx 代理，请确认 client.js 里的 wss path 已按 REVERSE_PROXY_PATH 修改。"
