#!/bin/bash
# 将 rsshub 的 nginx 片段写入 nginx 配置目录，不覆盖原机 site。
# 用户只需在自己的 site 里加一行 include。

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SNIPPET_SRC="$SCRIPT_DIR/rsshub-snippet.conf"
NGINX_SNIPPET="/etc/nginx/snippets/rsshub.conf"

# 由 docker_rsshub.sh 传入或使用默认值
RUN_PORT="${RUN_PORT:-1200}"
REVERSE_PROXY_URL="${REVERSE_PROXY_URL:-/rsshub/}"
...
echo "在要用 rsshub 的那个 server { } 内加一行："
echo ""
echo "    include /etc/nginx/snippets/rsshub.conf;"
echo ""
